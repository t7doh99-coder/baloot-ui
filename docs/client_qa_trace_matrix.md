...
| Sawa “forced Hakam” | §4.4 Sawa matches bid; buyer unchanged | Bidding `BidAction.sawa`: `bidding_manager.dart`. UI label **“Sawa · Bid”** vs in-play `Tooltip` on hands Sawa: `game_table_screen.dart`, `human_player_majlis_bar.dart`. |
...
3. **Bidding Sawa** — Round 1: opponent bids Hakam; as defender tap **“Sawa · Bid”**; confirm contract locks to Hakam with original Hakam bidder as buyer (per §4.4). Do **not** confuse with in-play Sawa (tooltip on bottom bar).
