      ...
      child: Stack(
        ...
        children: List.generate(cardCount, (index) {
          ...
          final compactArcLift = compact && direction == Axis.horizontal
              ...
              : 0.0;
          // Smooth parabolic arc for large bottom hand cards — center card
...
class _CenterTrickZoneState extends State<_CenterTrickZone>
  ...
  static const Map<_SeatAlignment, Offset> _throwStartOffsets = {
    ...
    // Start deep from user's hand zone — matches the actual card position
  ...
  // Dynamic X start offset for the bottom throw — set per-throw based on
  // which card in the fan was swiped.
  double _bottomThrowStartDx = 0.0;
  // Dynamic start angle — matches the card's tilt in the hand fan.
      ...
      for (final seat in _allSeats)
        seat: AnimationController(
          ...
          ..addStatusListener((status) {
            ...
            // appears — prevents the "card disappears" gap.
  ...
  void _applyPermanentImpactOnLanding() {
    ...
    for (int i = 0; i < totalCount - 1; i++) {
      ...
      // Strong angle rotation — each card rotates differently.
  ...
  void didUpdateWidget(covariant _CenterTrickZone oldWidget) {
    ...
    for (final seat in _allSeats) {
      ...
      if (count > (_lastSeatCounts[seat] ?? 0)) {
        ...
        // in the fan was thrown — rightmost card starts right, etc.
        if (seat == _SeatAlignment.bottom) {
          ...
          // Match the card's tilt in the hand fan — rightmost tilts
        ...
        animatedChildren.add(
          AnimatedBuilder(
            ...
            builder: (context, _) {
              ...
              // ── Per-seat easing curves ──
              // Bottom: custom curve — slow peel from hand, fast flick
              ...
              // ── Parabolic arc (natural curve, not straight line) ──
              final distanceY = (targetDy - start.dy).abs();
              final arcStrength = _throwArcStrength[seat] ?? 0.18;
              final arcBoost = switch (seat) {
                _SeatAlignment.top => 1.28,
              ...
              // ── Position: smooth single-curve trajectory ──
              final dx =
                  start.dx + ((targetDx - start.dx) * travelT) + sideCurve;
              final dy = start.dy + ((targetDy - start.dy) * travelT) + arcY;

              // ── Angle: base interpolation + subtle flight spin ──
              // sin(π·t) peaks at midpoint — card spins slightly mid-flight
              ...
              // ── Depth scale: smooth bell curve (rises mid-flight) ──
              // sin(π·travelT) peaks at 0.5 → card "lifts toward camera"
              ...
              // back — single hard bump like slamming a card on felt.
              double thumpScale = 0.0;
              double thumpDy = 0.0;
              if (travelT > 0.85) {
                final p = (travelT - 0.85) / 0.15; // 0→1 in last 15%
                // Sharp dip then rebound: sin(π·p) peaks at p=0.5
              ...
              // ── Lift-off blend (bottom seat only) ──
            ...
            for (int throwIndex = 0;
              ...
              staticCards.add(
                TweenAnimationBuilder<double>(
                  ...
                  // Smooth one-way slide to final impact position — no bounce.
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: const RadialGradient(
              ...
              radius: 1.5,
              colors: [
                Color(0xFF2A1510), // Deep antique maroon
                Color(0xFF140A07), // Almost black
              ],
            ),
            borderRadius: BorderRadius.circular(18.5), // Match outer minus padding
            border: Border.all(
              color: const Color(0x80E4C267), // Inner thin gold border
              width: 1,
            ),
          ),
              ...
              children: [
                ...
                Container(
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(
                        color: const Color(0xFFC49A3E).withValues(alpha: 0.3),
                        blurRadius: 12,
                        offset: const Offset(0, 4),
                      ),
                    ],
          ...
          Row(
            ...
            children: const [
              Text(
                'اللعبة : صن',
                style: TextStyle(
                  color: Color(0xFFEADBCE), // Warm off-white parchment
                  fontSize: 15,
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildClassicDivider(),
          _buildDataRow('الابناط', '49', '101', isBold: true),
          _buildClassicDivider(),
