...
void main() {
  // Dealer at seat 0 → first bidder is seat 3 (dealer's right, counter-clockwise)
  // Turn order: 3 → 2 → 1 → 0
  ...
  group('Round 1 — Hakam or Pass', () {
    test('player bids Hakam → becomes buyer with buyer card suit as trump', () {
      ...
      expect(bm.currentBidder, 3);
      bm.placeBid(3, BidAction.hakam); // seat 3 bids Hakam
    ...
    test('wrong seat throws InvalidBidException', () {
      ...
      expect(
        () => bm.placeBid(0, BidAction.pass), // seat 0, but bidder is seat 3
    ...
    test('invalid action in Round 1 throws', () {
      ...
      expect(
        () => bm.placeBid(3, BidAction.secondHakam, secondHakamSuit: Suit.spades),
        throwsA(isA<InvalidBidException>()),
      );
    });
  });
  ...
  group('Round 1 — Sawa', () {
    test('Sawa locks existing Hakam bid and ends immediately', () {
      ...
      expect(bm.isFinished, true);
      expect(bm.result!.mode, GameMode.hakam);
      expect(bm.result!.buyerIndex, 3); // original bidder
      expect(bm.result!.trumpSuit, Suit.hearts);
    });
  ...
  group('Round 2 — Sun, Second Hakam, Ashkal, Pass', () {
    ...
    test('Second Hakam with same suit as buyer card throws', () {
      ...
      expect(
        () => bm.placeBid(3, BidAction.secondHakam,
  ...
  group('Ashkal', () {
    ...
    test('Dealer (seat 0) can bid Ashkal', () {
      final bm = _toRound2();
      // Advance to dealer's turn (seat 0)
      bm.placeBid(3, BidAction.pass);
      bm.placeBid(2, BidAction.pass);
      bm.placeBid(1, BidAction.pass);
    ...
    test('Sane (seat 1, dealer left) can bid Ashkal', () {
      final bm = _toRound2();
      bm.placeBid(3, BidAction.pass);
      bm.placeBid(2, BidAction.pass);
      bm.placeBid(1, BidAction.ashkal); // Sane = dealer(0) + 1
    ...
    test('non-dealer/non-sane cannot bid Ashkal', () {
      ...
      expect(
        () => bm.placeBid(3, BidAction.ashkal), // seat 3 is not dealer or sane
        throwsA(isA<InvalidBidException>()),
      );
    });
  });
  ...
  group('Turn order verification', () {
    test('dealer at seat 2: bidding starts at seat 1, goes 1→0→3→2', () {
      final bm = BiddingManager(
        dealerIndex: 2,
        buyerCard: buyerCard,
      );

      expect(bm.currentBidder, 1); // dealer's right = (2+3)%4 = 1
      bm.placeBid(1, BidAction.pass);
      expect(bm.currentBidder, 0); // (1+3)%4 = 0
      bm.placeBid(0, BidAction.pass);
      expect(bm.currentBidder, 3); // (0+3)%4 = 3
      bm.placeBid(3, BidAction.pass);
      expect(bm.currentBidder, 2); // (3+3)%4 = 2
    });
  });
